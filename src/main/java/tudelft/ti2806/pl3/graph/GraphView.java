...
/**
 * Render the sequence graph. Created by Boris Mattijssen on 30-04-15.
 */
public class GraphView extends JPanel {
	...
	/**
	 * Construct a graph view containing nodes and edges. The constructor will
	 * automatically make the graph render.
	 * 
	 * @param nodes
	 *            the graph's nodes
	 * @param edges
	...
	public void renderGraph() {
		...
		for (Node node : nodes) {
			org.graphstream.graph.Node graphNode = graph.addNode(node.getId()
					+ "");
			graphNode.addAttribute("Node", node);
		}
		for (Edge edge : edges) {
			graph.addEdge(edge.toString(), edge.getFrom().getId() + "", edge
					.getTo().getId() + "");
		}
	...
	/**
	 * When the graph file was not found, show this message instead of the
	 * graph.
	 */
