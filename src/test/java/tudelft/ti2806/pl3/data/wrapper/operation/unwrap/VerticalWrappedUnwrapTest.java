...
public class VerticalWrappedUnwrapTest {
	...
	public void testBottomNode() {
		...
		assertTrue(((WrapperClone) bottom).getDataNodeList().contains(dataNode3));
		assertTrue(((WrapperClone) bottom.getIncoming().get(0)).getDataNodeList().contains(dataNode1));
	}

	@Test
	public void testRight1Node() {
		Wrapper right1 = unwrap.getResult().getOutgoing().get(0).getOutgoing().get(0);
		assertTrue(right1 instanceof WrapperClone);
		assertEquals(2, right1.getIncoming().size());
		assertEquals(0, right1.getOutgoing().size());
		assertTrue(((WrapperClone) right1).getDataNodeList().contains(dataNode4));
		assertTrue(((WrapperClone) right1.getIncoming().get(0)).getDataNodeList().contains(dataNode2));
		assertTrue(((WrapperClone) right1.getIncoming().get(1)).getDataNodeList().contains(dataNode3));
	}

	@Test
	public void testRight2Node() {
		Wrapper right2 = unwrap.getResult().getOutgoing().get(1).getOutgoing().get(0);
		assertTrue(right2 instanceof WrapperClone);
		assertEquals(2, right2.getIncoming().size());
		assertEquals(0, right2.getOutgoing().size());
		assertTrue(((WrapperClone) right2).getDataNodeList().contains(dataNode4));
		assertTrue(((WrapperClone) right2.getIncoming().get(0)).getDataNodeList().contains(dataNode2));
		assertTrue(((WrapperClone) right2.getIncoming().get(1)).getDataNodeList().contains(dataNode3));
	}
